...
namespace GinjaGaming.FinalCharacterController
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        private SkinnedMeshRenderer[] _firstPersonHiddenRenderers;
        #endregion
        ...
        private void Awake()
        {
            ...
            _firstPersonHiddenRenderers = GetRenderersWithTag("FirstPersonHidden");

            Cursor.lockState = CursorLockMode.Locked;
        }

        public SkinnedMeshRenderer[] GetRenderersWithTag(string tag)
        {
            // 1. Find all GameObjects with the specified tag.
            GameObject[] taggedObjects = GameObject.FindGameObjectsWithTag(tag);

            // --- LINQ Approach (Concise) ---

            // 2. Use LINQ to select the MeshRenderer component from each GameObject.
            // 3. Filter out any null results (in case a tagged object doesn't have a MeshRenderer).
            // 4. Convert the result to an array.
            return taggedObjects
                   .Select(go => go.GetComponent<SkinnedMeshRenderer>()) // Get the MeshRenderer from each GameObject
                   .Where(mr => mr != null) // Keep only those that actually have a MeshRenderer
                   .ToArray(); // Convert the filtered collection to an array
        }
        #endregion
        ...
        {
            _playerCamera.enabled = !_playerLocomotionInput.ThirdPersonToggleOn;
            foreach (SkinnedMeshRenderer mesh in _firstPersonHiddenRenderers)
            {
                mesh.shadowCastingMode = !_playerLocomotionInput.ThirdPersonToggleOn ? ShadowCastingMode.ShadowsOnly : ShadowCastingMode.On;
            }
        }
        #endregion
